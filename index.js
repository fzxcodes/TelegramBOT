...
prefix,
Id,
} = JSON.parse(fs.readFileSync(`./seting.json`)) // seting semua di seting.json

if (bot_token == "") {
    return console.log("=== isi bot token! ===")
}
...
bot.on("message", async(lol) => {
    try {
        ...
        if (!isGroup && !isCmd) console.log(chalk.whiteBright("├"), chalk.cyanBright("[ private chat ]"), chalk.whiteBright(typeMessage), chalk.greenBright("from"), chalk.whiteBright(user.username))
        if (isGroup && !isCmd) console.log(chalk.whiteBright("├"), chalk.cyanBright("[  grup  ]"), chalk.whiteBright(typeMessage), chalk.greenBright("from"), chalk.whiteBright(user.username), chalk.greenBright("in"), chalk.whiteBright(groupName))
   ...
   try {
   	result = await fetchJson(`https://api.xteam.xyz/dl/play?lagu=${query}&APIKEY=${xteamkey}`)
   	await lol.replyWithAudio({url: ${result.url}})
   } catch(e) {
   reply('Maaf error! ketik /report teks untuk lapor')
   console.log('ERROR | play error start at lie 171')
   break
...
//REGISTRATION FEATURES        
		...
		case 'report':
			...
			if (!isUser) return reply(mess.ser)
			try { 
				await bot.telegram.sendMessage(Id, query)
...
bot.telegram.getMe().then((getme) => {
    ...
    console.log(chalk.greenBright(' ===================================================='))
    console.log(chalk.whiteBright('╭─── [ BY FAIZ  ]'))
})
